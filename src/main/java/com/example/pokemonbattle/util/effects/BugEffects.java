...
public class BugEffects {
    ...
    public void createImpactEffect(double startX, double startY,
        ...
        switch (moveName) {
            ...
            case "x-scissor"    -> addScissorCut(startX, startY, endX, endY, intensity, timeline);
            ...
            case "infestation"  -> addSwarmStrike(endX, endY, intensity, timeline);
    ...
    private void addScissorCut(double sx, double sy, double ex, double ey, double intensity, Timeline timeline) {
        ...
        for (int i = 0; i < trailCount; i++) {
            ...
            KeyFrame fade = new KeyFrame(Duration.millis(delay + 170),
    ...
    private void addTwinNeedle(double sx, double sy, double ex, double ey, double intensity, Timeline timeline) {
        ...
        double perpY =  dx / Math.max(Math.sqrt(dx * dx + dy * dy), 1) * 8;
        ...
        for (int n = -1; n <= 1; n += 2) {
            ...
            int delay = (n < 0) ? 30 : 90;
            KeyFrame appear = new KeyFrame(Duration.millis(delay),
                    new KeyValue(needle.opacityProperty(), 0.9));
            KeyFrame fade = new KeyFrame(Duration.millis(delay + 200),
                    new KeyValue(needle.opacityProperty(), 0));
    ...
    private void addBugBite(double sx, double sy, double ex, double ey, double intensity, Timeline timeline) {
        ...
        for (int i = 0; i < streakCount; i++) {
            ...
            int delay = (int) (t * 120);
            KeyFrame appear = new KeyFrame(Duration.millis(delay),
                    new KeyValue(dot.opacityProperty(), 0.85));
    ...
    private void addLeechLife(double sx, double sy, double ex, double ey, double intensity, Timeline timeline) {
        ...
        for (int i = 0; i < orbCount; i++) {
            ...
            KeyFrame drain = new KeyFrame(Duration.millis(delay + 220),
    ...
    private void addSwarmStrike(double x, double y, double intensity, Timeline timeline) {
        ...
        for (int i = 0; i < swarmCount; i++) {
            ...
            int delay = i * 22;
            KeyFrame appear = new KeyFrame(Duration.millis(delay),
                    new KeyValue(bug.opacityProperty(), 0.9));
            ...
            KeyFrame fade = new KeyFrame(Duration.millis(delay + 340),
                    new KeyValue(bug.opacityProperty(), 0));
    ...
    private void addBugBuzz(double sx, double sy, double ex, double ey, double intensity, Timeline timeline) {
        ...
        for (int i = 0; i < ringCount; i++) {
            ...
            timeline.getKeyFrames().addAll(appear, travel, expand, fade);
    ...
    private void addSignalBeam(double sx, double sy, double ex, double ey, double intensity, Timeline timeline) {
        ...
        for (int b = 0; b < beamColors.length; b++) {
            ...
            int beamDelay = b * 18;
            KeyFrame bAppear = new KeyFrame(Duration.millis(beamDelay),
                    new KeyValue(beam.opacityProperty(), 0.85));
            KeyFrame bExtend = new KeyFrame(Duration.millis(beamDelay + 200),
                    new KeyValue(beam.endXProperty(), ex),
                    new KeyValue(beam.endYProperty(), ey));
            KeyFrame bFade = new KeyFrame(Duration.millis(beamDelay + 340),
    ...
    private void addSilverWind(double sx, double sy, double ex, double ey,
        ...
        for (int i = 0; i < count; i++) {
            ...
            KeyFrame drift = new KeyFrame(Duration.millis(delay + 220),
    ...
    private void addPollenPuff(double sx, double sy, double ex, double ey,
        ...
        KeyFrame pAppear = new KeyFrame(Duration.millis(20),
                new KeyValue(puff.opacityProperty(), 1.0));
        KeyFrame pArc = new KeyFrame(Duration.millis(160),
                new KeyValue(puff.centerXProperty(), midX),
                new KeyValue(puff.centerYProperty(), midY));
        KeyFrame pLand = new KeyFrame(Duration.millis(280),
                new KeyValue(puff.centerXProperty(), ex),
                new KeyValue(puff.centerYProperty(), ey));
        KeyFrame pBurst = new KeyFrame(Duration.millis(360),
                new KeyValue(puff.opacityProperty(), 0),
                new KeyValue(puff.radiusProperty(), orbRadius * 2));

        timeline.getKeyFrames().addAll(pAppear, pArc, pLand, pBurst);
        ...
        for (int i = 0; i < pollenCount; i++) {
            ...
            int delay = 280 + i * 18;
            KeyFrame appear = new KeyFrame(Duration.millis(delay),
                    new KeyValue(pollen.opacityProperty(), 0.85));
            ...
            KeyFrame fade = new KeyFrame(Duration.millis(delay + 280),
                    new KeyValue(pollen.opacityProperty(), 0));
