    ...
    private void processNextDamageMessage() {
        ...
        Runnable afterAnimation = () -> {
            ...
            if (msg.isTargetFainted()) {
                ...
                if (targetIsMe) {
                    // Player's pokemon fainted — show forced switch overlay if replacements exist
                    List<PokemonInstance> available = player.getTeam().stream()
                            .filter(p -> !p.isFainted())
                            .toList();
                    if (!available.isEmpty()) {
                    ...
                    if (next != null) {
                        ...
                        String faintEntry = cap(targetPok.getName()) + " fainted! " + cap(target.getName())
                                + " sends out " + cap(next.getName()) + "!";
