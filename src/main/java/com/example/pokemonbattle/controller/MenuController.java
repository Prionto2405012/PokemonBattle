...
public class MenuController {
    ...
    // ── Glow / selector animations (unchanged) ───────────────────
    ...
    private void playButtonEntrance() {
        ...
        for (int i = 0; i < ordered.length; i++) {
            ...
            slide.setFromX(slideDist); slide.setToX(0);
    ...
    public void initialize() {
        ...
        if (loaded != null) System.out.println("[MenuController] Font: " + loaded.getFamily());
        else System.err.println("[MenuController] Font failed to load");
        ...
        // Start BGM only if nothing is already playing (avoids restarting on scene switch back)
    ...
    private void setupButtonHover() {
        for (int i = 0; i < menuButtons.size(); i++) {
            final int index = i;
            Button btn = menuButtons.get(i);
            btn.setOnMouseEntered(e -> { if (index != selectedIndex) btn.getStyleClass().add("menu-hovered"); });
    ...
    @FXML void onSettingsButtonClick() {
        try {
